...
There is **no** UI state named `waiting` / `expired` / `denied` / `revoked` on the Welcome path. Handoff `expires_at` is documented as a 5-minute TTL, not cert expiry (`PubkyRingAuthService.certFromHandoffAppKey`). Denied/expired Ring approval never returns to AwaitingRingAuth with a status — the user stays on “Waiting…” until Cancel or a successful callback.
