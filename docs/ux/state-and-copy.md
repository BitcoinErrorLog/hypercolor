...
| Keystore unavailable | `useSessionStatusStore.kind === 'keystore-unavailable'` (`sessionStatusStore.ts`, `sessionUi.ts`) | `initKeyStore` still pending (4s fail-open / splash Continue) or init failed; `KeyStore.isInitialized()` is false | Banner: `Encrypted storage is not ready on this device.` / `Messaging is paused. Local history is untouched.` (`COPY.keystoreUnavailable`). No Enable / Try again / sign-out action (`MainTabs.tsx` SessionBannerHost). UI must not read or write secrets. |
...
**Boot reconcile is report-only.** After `initKeyStore()` (which runs the v1→v2 MMKV migration), `LinkService.reconcileAdoptedSessionsAtBoot` asks native to compare adopted Paykit session aliases against the KeyStore alias. First unowned sighting writes a quarantine record (boot counter + process token) and leaves the native bearer in place. A later OS-process sighting of the same unowned alias logs `PaykitLinkReconcile subsequent-sighting alias=<opaque-id>` and keeps the bearer and the record. `session()` still refuses unknown aliases. Nothing in this path deletes a native bearer or a KeyStore entry. Pending leftovers that were never adopted are still swept at process start.

**MMKV wrong-key / discard.** `react-native-mmkv` has no per-key error channel. A wrong encryption key discards the whole instance (`OnErrorDiscard`) and continues as an empty writable store. Init then installs a canary and reports ready-and-empty: `pubky` is gone too, so `hasPersistedSession()` is false and the user is logged out. Readiness does not protect the bearer; the instance-wide discard does. Unreadable is indistinguishable from a fresh install. The v2 MMKV key is 96 bits (HKDF-SHA256 → base64url, 16 UTF-8 bytes) because nitro marshals the key as UTF-8 and MMKV keeps 16 bytes.
...
**Revoked / expired session:** `LinkService.restorePersistedSession` comment: native `auth` rejection (revoked/expired) **deletes the stored alias**; `network` keeps it (`session-offline`). There is no “revoked” banner. Next Enable Messaging visit is `needs-enable` or `session-offline`. Coarse native messages (`PaykitLinkNative.ts` `COARSE_NATIVE_MESSAGES`): `authentication failed`, `network error`, `protocol error`, `resource consumed`, `validation failed`, `unavailable`, `auth flow cancelled`. Native Ring-auth flow: `startAuthFlow` → `awaitAuthApproval(flowId)` → `stopAuthKeepalive(flowId)`. Native admits one owner lease per live `flowId`. A second `awaitAuthApproval` while reserved, awaiting, committing, or still settling a cancellation rejects `validation` / `already awaiting` and must not prune the owner's tombstone. Discarded flows call `cancelAuthFlow(flowId)` (Paykit FFI has no auth-flow cancel primitive; native discard cancels the await job, drops a not-yet-awaited UniFFI flow so its relay subscription stops, stops keepalive, and tombstones the id). Cancel-before-await: the later `awaitAuthApproval` rejects `auth_flow_cancelled` and that caller is the owner who prunes the tombstone. A wait already spawned by `awaitApproval` runs to completion inside Paykit and cannot be aborted; after that FFI return native must not persist, must `close()` the handle exactly once, and rejects `auth_flow_cancelled`. Approval persist and JS resolve are one lock-linearized commit: the owner slot stays teardown-visible as `committing(lease)` until the session is written and the await promise settles. If invalidation wins that race, native writes nothing, does not resolve, and rejects `unavailable`. If the commit wins, teardown must not roll back the adopted session. Bridge/module invalidation and coroutine-scope teardown reject `unavailable` (not `auth_flow_cancelled`) and never persist; after invalidation, `startAuthFlow` / `awaitAuthApproval` / `cancelAuthFlow` reject `unavailable` immediately. `clearAllNativeSecrets` (sign-out) is not sticky teardown: it closes idle flows immediately, cancels admitted owners so only their `finally` closes, and in-flight await rejects `auth_flow_cancelled` (user discard) without persisting; a later `startAuthFlow` still succeeds. Close is exact-once: idle / cancel-before-await / clear-all idle close immediately; the admitted owner closes after FFI settles (success, failure, cancel-during-await, teardown, or clear-all). `LinkService.requireSessionAlias` throws `auth` / `Enable encrypted messaging to write to your homeserver.`
