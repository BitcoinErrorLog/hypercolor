...
## Mounted catalog (Wave 3 Part 2)

When `E2E_VRT=1` or `EXPO_PUBLIC_E2E_VRT=1` in a `__DEV__` build, `App.tsx` mounts `vrt/VrtCatalogRoot` instead of the product navigator. The catalog module is `require()`’d only inside that gate so release builds keep it dead.

Scene selection:

- Deep link / clipboard channel: `hypercolor://e2e/vrt?scene=<catalog-id>`
- `setVrtScene(id)` from the e2e harness

### Scene marker assertion (required)

Every catalog mount renders an on-canvas accessibility node whose `testID` and
`accessibilityLabel` are exactly `vrt-scene:<catalog-id>` (for example
`vrt-scene:auth.welcome.idle`). Maestro flows **must** `assertVisible` that
marker **before** `takeScreenshot`. A mismatch fails the scene; it must not
capture the default token-swatch (or any other) screen.

Android: `scripts/vrt-capture-android.sh` launches the app **once per viewport**,
then writes the HC_E2E cmd **after** launch (launching first used to wipe the
cmd file and leave every capture on token-swatch). iOS:
...
After capture, `tsx vrt/integrityGate.ts <baselineDir> [markerLedger] [reportJson]`
compares every pair of **different** scenes on the same platform+viewport.

Similarity is **content-aware**: near-canvas `#0A0A0A` pixels shared by both
images are excluded from the denominator so tall dark screens don’t false-fail
at full-frame 99%. The run fails when content-pixel identity is ≥99%, or when
the full frame is ≥99.95% identical (near byte-clone). Failures print both
filenames. The marker ledger
(`vrt/output/report/marker-ledger-{android,ios}.json`) must list every expected
scene as asserted; a missing assert fails the gate even when PNGs exist.

```bash
npm run vrt:capture:android
npm run vrt:capture:ios
npx tsx vrt/integrityGate.ts vrt/baselines/android vrt/output/report/marker-ledger-android.json vrt/output/report/integrity-android.json
npx tsx vrt/integrityGate.ts vrt/baselines/ios vrt/output/report/marker-ledger-ios.json vrt/output/report/integrity-ios.json
```
...
`StatusBar.setHidden` under VRT; iOS `simctl status_bar override`). The e2e
clipboard toast (`HC_E2E_DONE:…`) is suppressed in VRT builds — file channel
only.

Maestro flows under `.maestro/vrt/generated/` are regenerated by
`npm run vrt:catalog`. Flat capture baselines live under
`vrt/baselines/{android,ios}/`.
