...
function xorPad(key: Uint8Array, pad: number): Uint8Array {
  ...
  out.set(key);
  for (let i = 0; i < 64; i += 1) out[i] = (out[i] ?? 0) ^ pad;
  return out;
}

/** Hex HMAC-SHA-256 of UTF-8 `message` under a hex key. */
export function hmacSha256Hex(keyHex: string, message: string): string {
  ...
  inner.set(xorPad(key, 0x36));
  inner.set(msg, 64);
  const outerMsg = sha256(inner);
  const outer = new Uint8Array(64 + outerMsg.length);
  outer.set(xorPad(key, 0x5c));
  outer.set(outerMsg, 64);
